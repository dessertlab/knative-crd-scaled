...
// controller reconciling v1.RTResource.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1.RTResource. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1.RTResource) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1.RTResource.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1.RTResource. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1.RTResource) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1.RTResource if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1.RTResource.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1.RTResource) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1.RTResource) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1.RTResource resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister rtresourcev1.RTResourceLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.RTResources(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, logger *zap.SugaredLogger, existing *v1.RTResource, desired *v1.RTResource) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.RtresourceV1().RTResources(desired.Namespace)
		...
		updater := r.Client.RtresourceV1().RTResources(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1.RTResource, desiredFinalizers sets.Set[string]) (*v1.RTResource, error) {
...
func (r *reconcilerImpl) updateFinalizersFilteredServerSideApply(ctx context.Context, resource *v1.RTResource, desiredFinalizers sets.Set[string]) (*v1.RTResource, error) {
	...
	patcher := r.Client.RtresourceV1().RTResources(resource.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFilteredMergePatch(ctx context.Context, resource *v1.RTResource, desiredFinalizers sets.Set[string]) (*v1.RTResource, error) {
	// Don't modify the informers copy.
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1.RTResource) (*v1.RTResource, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1.RTResource, reconcileEvent reconciler.Event) (*v1.RTResource, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
	...
	if err != nil {
		...
		getter := r.Client.RtresourceV1().RTResources(resource.Namespace)
